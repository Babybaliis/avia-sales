...
const initialState: ReducerInitialState = {
    tickets: [] as Tickets[],
    filters: [
        {label: "Все", done: false, id: 1, stopCount: -1},
        {label: "Без пересадок ", done: false, id: 2, stopCount: 0},
        {label: "1 пересадка", done: false, id: 3, stopCount: 1},
        {label: "2 пересадки", done: false, id: 4, stopCount: 2},
        {label: "3 пересадки", done: false, id: 5, stopCount: 3},
    ],
};
...
export const ticketSlice = createSlice({
    ...
    reducers: {
        ...
        onClick(state, action: PayloadAction<Filter>) {
            ...
            state.filters[0].done =
                state.filters.filter((filter) => filter.id !== 1 && filter.done)
                    .length === 4;
        },
    },
});
